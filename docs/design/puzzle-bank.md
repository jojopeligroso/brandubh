...
**Named set**. 150 of 158 are in the **Pool**, which is the proportion ADR-0003
predicted.
...
Only `regicide` and `escape` are proofs, so it runs over 13 of 158 records and
...
scripted reply, and 157 of 158 lines hold a single solver move with no reply. It
...
separate two puzzles has not said which is harder. Under the shipped weights 89
of 158 puzzles grade at exactly 30, so this is not a rounding detail: a
...
`lineLength` as 1 on 157 of 158 puzzles, so the column is nearly constant and no
