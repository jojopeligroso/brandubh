...
export default function TablutScreen({
  ...
  const sideName = (side: Side): string =>
    side === "defenders" ? t.tablutWhite : t.tablutBlack;
  const seatName = (side: Side): string =>
    side === aiSide ? t.tablutDifficulties[difficulty] : sideName(side);
  ...
  const menuItems = [
    ...
    { label: t.tablutRestart, onClick: requestRestart },
    ...(atTip && !gameOver && tip >= 1 ? [{ label: t.tablutUndo, onClick: takeback }] : []),
