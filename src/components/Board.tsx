...
interface BoardProps {
  ...
  /**
   * The travel animation has finished. Reported from the animation itself
   * rather than timed alongside it: the two clocks start a frame or more apart
   * (see AI_SLIDE_BACKSTOP_MS in src/useAiReveal.ts), and a stone taken down on
   * the wrong one stops short of its square.
   */
  onAiSlideEnd?: () => void;
          ...
          {(() => {
            ...
            return (
              <div
                ...
                className="ai-mover"
                // The stone's travel and its lift are two animations on nested
                // elements, and the child's bubbles up here too — so the name
                // has to be checked, not just the event.
                onAnimationEnd={(e) => {
                  if (e.animationName === "ai-slide") onAiSlideEnd?.();
                }}
                style={
                  {
                    "--fr": from.row,
