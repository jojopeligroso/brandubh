...
// module (2026-09-09), including the Tablut twin it names below. Brandubh's
// `../solver.ts` is wired into `puzzleBank.test.ts`, `scripts/genpuzzles.ts`,
// `scripts/handadds.ts` and `scripts/solve.ts`; neither larger board's solver
// has any such caller, so the "independent oracle" role described below is not
// currently exercised here. Not deleted — see `TASKS.md` ("Tablut and
// Copenhagen parity") — because wiring it in (recognizer cross-validation,
// puzzle mining) is the point, not removing it.
//
