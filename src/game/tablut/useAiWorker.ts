...
/**
 * The Tablut side's worker hooks — play and analysis, in one file because they
 * are the same twenty lines twice over and the reason they are *separate worker
 * instances* is the only interesting thing about either of them.
 *
 * That reason, unchanged from `../useAiWorker.ts` / `../useAnalysisWorker.ts`:
...
/** Runs Tablut move selection off the main thread. */
export function useAiWorker(): {
  requestMove: (state: GameState, difficulty: Difficulty, rules: TablutRuleSet) => Promise<AiMove>;
  cancel: () => void;
} {
  const { send, cancel, supported } = useWorkerChannel();
  const requestMove = useCallback(
    (state: GameState, difficulty: Difficulty, rules: TablutRuleSet): Promise<AiMove> =>
      supported
...
export function useAnalysisWorker(): {
  requestAnalysis: (state: GameState, rules: TablutRuleSet, deep?: boolean) => Promise<AiMove>;
  cancel: () => void;
} {
  const { send, cancel, supported } = useWorkerChannel();
  const requestAnalysis = useCallback(
    (state: GameState, rules: TablutRuleSet, deep = false): Promise<AiMove> =>
      supported
