...
// particular — being the *independent oracle* the engine's recognizers are
// cross-validated against, in the way `../recognizers.test.ts` validates
// Brandubh's. A recognizer and the search it feeds must never be each other's
// only witness.
//
// It solves less of Tablut than its twin solves of Brandubh, and by a wide
// margin: 81 squares, 25 pieces and roughly three times the branching factor put
// the full game far out of reach (Brandubh's own opening is already unsolved —
// see docs/solving.md). What it does reach is near-terminal positions and thinned
// endgames, which is exactly what a soundness test needs.
//
// A sound, *partial* solver for Tablut positions. It reuses the authoritative
// engine (allMoves / applyMove / status) so the rules it solves under are exactly
// the rules the game plays under — including threefold repetition, which the
// engine derives from move history. Correctness over speed: a solver is only worth
// anything if it never lies.
//
...
//     only in repetition outcomes. `memo:false` disables the table for full rigor
//     on small positions; the default `memo:true` accepts this minor unsoundness
//     to make reduced-material and endgame solves tractable.
//   • This does not make the *full opening* solvable here — see docs/solving.md for
//     the state-space and throughput numbers. It solves near-terminal positions,
...
import type { TablutRuleSet as RuleSet } from "./variants";
