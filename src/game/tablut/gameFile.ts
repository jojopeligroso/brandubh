// ── Game file (PGN-style import / export, Tablut) ─────────────────────────────
// The Tablut twin of `../gameFile.ts`: the same PGN-derived shape wrapped around
// the notation `rules.ts` emits, over files a–i and ranks 1–9.
//
//   [Format "tablut-1"]
//   [Event "Tablut"]
//   [Variant "tablut-gulo"]
...
//   0-1
//
// Two differences from the Brandubh format are worth stating, because both would
// silently misread a file otherwise:
//
//   • **White moves first** (baseline rule 2), so a numbered pair reads
//     "White Black" — the reverse of the Brandubh file, where the attackers lead.
//   • **The `Rules` tag carries enums, not just flags.** Tablut's ruleset has four
//     non-boolean rules (`escape`, `firstMove`, `throneBlocks`, `throneAnvil`)
//     alongside `repetitionResult`, and each is written as a plain string. The
//     writer and reader are both derived from the defaults, so a rule added later
//     is carried by the format for free whichever kind it is.
//
// The two formats are deliberately unrelated: a `tablut-1` file is not a
// `brandubh-1` file with a bigger board, and neither parser will accept the
// other's moves — `e7-b7` is off the edge of a 7x7 board, and the variant ids do
// not overlap.
//
 ...
 * value, so a rule added to `TablutRuleSet` later is carried by this format
...
/** Filename for a downloaded export: `tablut-<variant>-<date>.tafl`. */
...
const MOVE_RE = /^([a-i])([1-9])[-–—x]?([a-i])([1-9])(x(\d*))?[!?+#]*$/i;
...
/** Parse a PGN-style Tablut game file. Never throws. */
...
function resolveVariant(tag: string): string | "unknown" {
  ...
  for (const r of Object.values(VARIANTS)) {
    ...
    // "Tablut · corner escape" → matched by name above; this catches
    // "tablut corners" and similar space-separated spellings of an id.
    if (v.replace(/[·.\s]+/g, " ").endsWith(` ${r.id}`)) return r.id;
  }
  // The shorthands people actually type. "tablut" alone is the baseline, which is
  // also why it is checked after the exact-id match above rather than before.
  if (v.includes("gulo") || v.includes("dimetr") || v.includes("2025")) return "tablut-gulo";
  if (v.includes("corner")) return "tablut-corners";
  if (v.includes("tournament") || v.includes("aage") || v.includes("nielsen")) return "tablut-aage";
  if (v.includes("linnaeus") || v.includes("smith") || v.includes("tablut")) return "tablut";
  return "unknown";
}
