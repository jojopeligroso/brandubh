// Runs Tablut engine searches off the main thread so `hard`'s budget never
// freezes the board. Bundled into the static build by Vite
// (`new Worker(new URL(...))`), so it ships and runs 100% offline.
//
// This is a separate worker *module* from Brandubh's, not just a separate
// instance, and it has to be: the transposition table is module state in
// `engine.ts`, and the two games' tables must never meet. They key positions by
// `hashBoard`, whose string is 50 characters on a 7×7 board and 82 on a 9×9 one,
// so a collision is not the hazard — sharing a table between two different
// evaluation functions is. Two modules means two tables, with nothing to reason
// about.
//
// As on the Brandubh side, the two *kinds* of request share this module but never
...
ctx.onmessage = (e) => {
  const req = e.data;
  // GameState / TablutRuleSet / Move are plain data, so they cross the worker
