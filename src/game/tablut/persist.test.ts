...
describe("the storage key", () => {
  it("is Tablut's own, so the two games never clobber each other", () => {
    // The reason there is a second key at all. A player mid-game in Brandubh who
    // opens Tablut, plays, and comes back must still find their Brandubh game.
    expect(GAME_STORAGE_KEY).not.toBe(BRANDUBH_KEY);
    expect(GAME_STORAGE_KEY).toBe("tablut.game.v1");
  });

  it("leaves a Brandubh save untouched when a Tablut game is saved and cleared", () => {
...
describe("save then restore", () => {
  it("rebuilds the timeline move for move", () => {
    ...
    expect(restored!.rules).toEqual(baseline);
  });
...
describe("a save that cannot be trusted is dropped, not half-restored", () => {
  ...
  it("refuses unparseable text and an unknown variant", () => {
    ...
    const raw = JSON.parse(serializeGame(snapshotOf(play(SHORT)))) as SavedGame;
    expect(parseAt(JSON.stringify({ ...raw, variantId: "hnefatafl" }))).toBeNull();
  });
  ...
  it("refuses a game replayed under a ruleset it was not played under", () => {
    ...
    const capture = ["f5-f6", "b5-b7", "e3-h3", "f1-f5"]; // f1-f5 takes one
