// ── Replay-and-validate (Tablut) ──────────────────────────────────────────────
// The Tablut twin of `../replay.ts`, and the same trust boundary: rebuilding a
...
// from the input.
//
// One difference from Brandubh worth stating: the opening position depends on the
// ruleset here, because `firstMove` is a flag (baseline rule 2 gives White the
// first move). Replaying under the wrong ruleset therefore fails at ply 0 rather
// than drifting quietly — which is the direction you want.
...
const plyText = (p: PlyInput): string => {
  const name = (s: Square): string =>
    s.row >= 0 && s.row < 9 && s.col >= 0 && s.col < 9 ? squareName(s) : "?";
 ...
 */
export function replayPlies(plies: PlyInput[], rules: TablutRuleSet): ReplayResult {
