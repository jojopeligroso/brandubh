// ── Position setup (FEN-equivalent, Tablut) ──────────────────────────────────
//
// The Tablut twin of `../position.ts`. Two things differ, and both come from the
// rules rather than from the board being bigger:
//
//   • Nine ranks of nine, so the digit run-length can reach 9 rather than 7.
//   • `isRestricted` and "has the king already won" both take the ruleset. A
//     baseline Tablut corner is an ordinary square a soldier may stand on, so
//     rejecting a soldier there — which is right for Brandubh — would refuse
//     perfectly legal positions here. And the king is home on any rim square,
//     not in a corner.
//
...
//     3AAA3/4A4/...  d
//
// Nine ranks, top (rank 9) first, separated by `/`, then the side to move.
...
import { isEscapeSquare, isRestricted, isThrone } from "./rules";
...
export type PositionErrorCode =
  /** Not nine ranks separated by `/`. */
  | "bad_rank_count"
  /** A rank that does not add up to nine squares. */
  | "bad_rank_width"
  ...
  | "king_count"
  /** A soldier standing on a square only the king may occupy — the throne always,
   *  and the corners only under a ruleset that restricts them. */
  | "restricted_square"
...
export interface PositionError {
  ...
  /** 1-based rank (9 down to 1 as written) when the problem is locatable. */
  rank?: number;
 ...
 */
export function parsePosition(text: string, rules: TablutRuleSet): PositionResult {
  ...
  if (sidePart === "a" || sidePart === "attackers") turn = "attackers";
    ...
    return {
      ...
      error: {
        code: "bad_side",
        token: parts[1] ?? "",
        detail: 'the side to move must be "a" (Black) or "d" (White).',
      },
    };
  }

  const structural = validateBoard(board, rules);
 ...
 */
export function validateBoard(board: Board, rules: TablutRuleSet): PositionError | null {
  ...
  for (let r = 0; r < BOARD_SIZE; r++) {
    for (let c = 0; c < BOARD_SIZE; c++) {
      ...
      // Which squares are the king's alone is a *rules* question here: the
      // throne always, the corners only when the variant restricts them. Under
      // the baseline a soldier on a corner is an entirely ordinary position.
      if (piece !== null && piece !== "king" && isRestricted(r, c, rules)) {
        return {
 ...
 *
 *  Under edge escape this is true of any rim square, so it fires far more often
 *  than the Brandubh version — which is correct, and worth knowing if you paste
 *  in a position with the king anywhere on the outside. */
export function kingIsHome(board: Board, rules: TablutRuleSet): boolean {
  for (let r = 0; r < BOARD_SIZE; r++) {
    for (let c = 0; c < BOARD_SIZE; c++) {
      if (board[r][c] === "king" && isEscapeSquare(r, c, rules)) return true;
    }
  }
  return false;
}
